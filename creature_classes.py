...
class Head_node(pygame.sprite.Sprite):
    def __init__(self, pos, display_surf, size, speed, groups):
...
class Body_node(Head_node):
    def __init__(self, pos, display_surf, size, speed, groups):
        super().__init__(pos, display_surf, size, speed, groups)
...
class LIZARD_node(Body_node):
    def __init__(self, pos, display_surf, size, speed, groups, has_legs=False):
        super().__init__(pos, display_surf, size, speed, groups)
        self.direction = pygame.Vector2()
        self.size = (size, size)
        self.image = pygame.Surface(self.size)
        ...
        if has_legs:
            ...
            for sign in [1, -1]:
                ...
                leg_bend = Leg_Node(pos=self.leg_initial_bend, display_surf=self.display_surf, size=self.leg_size, speed=self.speed, groups=self.leg_nodes_sprites)
                leg_endnode = Leg_Node(pos=self.leg_initial_endpoint, display_surf=self.display_surf, size=self.leg_size, speed=self.speed, groups=self.leg_nodes_sprites)
